    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if acc_s_te>acc_log:
                ...
                torch.save(
                    netF.state_dict(),
                    osp.join(args.output_dir, "target_F_NRC_BN_" + '2021_'+str(args.K) + ".pt"))
                torch.save(
                    netB.state_dict(),
                    osp.join(args.output_dir,
                                "target_B_NRC_BN_" + '2021_' + str(args.K) + ".pt"))
                torch.save(
                    netC.state_dict(),
                    osp.join(args.output_dir,
                                "target_C_NRC_BN_" + '2021_' + str(args.K) + ".pt"))
...
if __name__ == "__main__":
    ...
    for i in range(len(names)):
        ...
        args.out_file = open(
            osp.join(args.output_dir, "log_NRC_BN_{}.txt".format(args.tag)), "w"
        )
